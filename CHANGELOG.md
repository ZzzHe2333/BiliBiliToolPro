## 3.8.3
- Feature: 独立仓库青龙订阅使用严格 `Zzz_*` 配置隔离、任务互斥和统一青龙通知
- Feature: B 币券支持按账号充电配置、领取时间持久化、临期提醒与最后 48 小时自动处理
- Fix: Docker/Podman/Helm/SCF/青龙部署来源统一到本仓库，移除上游同步与不兼容旧部署入口
- Fix: 正式 Release、Git tag 与容器语义版本绑定到同一 commit，主分支镜像改用 `main-<commit>` 防止版本标签漂移
- Fix: 腾讯云 SCF 停止 Trim 发布并启用严格配置隔离
- Fix: 青龙默认不再改写整个容器的软件源；国内镜像改为显式 opt-in
- Doc: 修正本地发布包命名、默认任务状态、Issue 模板和独立仓库文档
## 3.8.2
- Fix[#1026]: 更新文档
## 3.8.1
- Fix[#1005]: 更新文档
