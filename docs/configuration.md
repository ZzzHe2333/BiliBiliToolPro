...
本文档以当前 `ZzzHe2333/BiliBiliToolPro` fork 的实际代码为准。历史版本中的 `DayOfAutoCharge`、`DayOfReceiveVipPrivilege`、`DailyTaskConfig.AutoChargeUpId` 等配置已经废弃，不应继续使用。
...
本 fork 的青龙订阅任务会自动开启 `Zzz_IsolatedMode=true`，此时只读取 `Zzz_*` 业务环境变量，并且不加载 `cookies.json`，用于避免与原版 `Ray_*` 配置串号。

Web/Docker 配置通常不需要前缀。
...
| 配置键 | 代码默认值 | Console 默认配置 | 说明 |
| --- | ---: | ---: | --- |
| `Security__IsSkipDailyTask` | `false` | `false` | 全局跳过任务 |
| `Security__RandomSleepMaxMin` | `30` | `30` | 随机睡眠最大分钟数，0 表示关闭 |
| `Security__IntervalSecondsBetweenRequestApi` | `3` | `20` | 对指定 HTTP 方法的最短请求间隔 |
| `Security__IntervalMethodTypes` | `GET,POST` | `GET,POST` | 需要应用间隔的方法 |
| `Security__WebProxy` | 空 | 空 | HTTP 代理 |
...
本 fork 不会因为充电检查任务每天运行就每天消费 B 币券。
...
| 配置节 | 默认启用 | 主要默认值 |
| --- | --- | --- |
| `MangaTaskConfig` | `true` | `CustomComicId=27355`, `CustomEpId=381662` |
| `MangaPrivilegeTaskConfig` | `true` | - |
| `Silver2CoinTaskConfig` | `true` | - |
| `VipPrivilegeConfig` | `true` | - |
| `VipBigPointConfig` | `true` | `ViewBangumis=33378` |
| `LiveLotteryTaskConfig` | `true` | 自动分组开启，奖品过滤规则见 appsettings |
| `LiveFansMedalTaskConfig` | `true` | 心跳 70 分钟，默认跳过 20 级及以上粉丝牌 |
| `UnfollowBatchedTaskConfig` | `true` | 分组 `天选时刻`，每次 20 个，保留 UID 默认空 |

每个任务都可以使用对应的：

```text
<Section>__IsEnable=true/false
```
...
- `Zzz_BILI_USE_CN_MIRROR`：中国大陆默认 `true`，首次安装依赖时优先使用国内系统软件源；
- `Zzz_BILI_LOCK_WAIT_SECONDS`：同一青龙容器中等待其他 Zzz-Bili 任务结束的最长秒数，默认 3600。

`bilitool` 模式使用本 fork 自己的 `fork-main` 滚动构建，并校验二进制构建 commit 与订阅仓库 HEAD，避免运行旧代码。
...
青龙订阅部署请优先使用青龙面板系统通知，不建议再为本 fork 重复配置一套 Serilog Server酱/PushPlus/Webhook，否则容易产生重复推送。
...
1. 青龙 fork 是否使用 `Zzz_` 前缀；
2. 配置层级是否用双下划线 `__`；
3. 是否存在更高优先级的同名配置；
4. 青龙定时问题是否错误地修改了 `TaskConfig__Cron`，而没有修改面板 Shell Cron；
5. 多账号充电的 `Accounts__<UID>` 是否填了真实 B站 UID；
