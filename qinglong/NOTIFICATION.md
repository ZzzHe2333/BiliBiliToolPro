...
BiliBiliToolPro 的多账号逻辑会尽量在单个账号失败后继续执行其他账号，因此主进程有时会返回 `0`，但日志中已经出现 `ERR`/`FTL`。

订阅包装脚本会在程序结束后再检查一次本次日志：

- 进程非 0：按原退出码判定失败；
- 进程为 0，且没有 Error/Fatal 日志：成功；
- 进程为 0，但存在 Error/Fatal 日志：标记为“部分失败”，返回退出码 `2`，青龙面板也会将本次运行显示为失败。

这样不会中途终止多账号任务，但也不会出现“某些账号明显报错，最后通知仍写成功”的情况。

通知正文来自本次任务输出。正常启动时会去掉 `dotnet run` 在程序启动前产生的编译信息，只保留 BiliBiliToolPro 运行日志；如果程序没有成功启动，则保留启动/编译错误用于排查。超长正文会保留开头和结尾并截断中间部分。

通知成功或失败本身不会覆盖 BiliBiliToolPro 的业务状态；最终退出码取主进程状态和 Error/Fatal 日志检测后的结果。
