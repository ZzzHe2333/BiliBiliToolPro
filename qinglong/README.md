...
| --- | --- | --- |
| 直播粉丝牌 | `21 0 * * *` | 每天 00:21 |
| 领取大会员福利 | `0 4 * * *` | 每天 04:00 |
| 大会员大积分 | `33 4 * * *` | 每天 04:33 |
| 每日漫画任务 | `0 5 * * *` | 每天 05:00 |
| 领取大会员每月漫画权益 | `10 5 * * *` | 每天 05:10 |
| 每日任务 | `38 5 * * *` | 每天 05:38 |
| 免费 B 币券充电 | `0 20 * * *` | 每天 20:00 |
| 测试 Cookie | `0 0 1 1 *` | 每年 1 月 1 日 00:00 |
| 扫码登录 | `0 0 1 1 *` | 每年 1 月 1 日 00:00 |
| 定时保护 | `1,10 2 * * *` | 每天 02:01、02:10 |
...
`Zzz-Bili 定时保护` 用于避免仓库订阅覆盖用户在青龙面板中手动修改过的业务任务时间：

1. **02:01 快照**：读取更新前仓库脚本中的默认 Cron，并与青龙面板当前 Cron 对比；只有两者不一致时，才认定该任务是用户手动修改过。
2. **02:02 更新仓库**：青龙正常执行 `Zzz-BiliBiliToolPro` 订阅。未手工修改过的任务允许跟随仓库新的默认时间。
3. **02:10 恢复**：只恢复 02:01 记录到的手工 Cron，不修改其他任务的时间。

因此：

- **没有手动改过时间**：以后仓库调整默认 Cron 时，任务会正常跟随新默认值；
- **手动改过时间**：以后更新订阅时，保留你在青龙面板设置的时间；
- 只保护 Cron，不修改任务命令、启用状态或其他属性；
- 首次拉取包含“定时保护”的版本前没有历史快照，因此第一次更新会按仓库当前默认时间落地；从保护任务完成一次 02:01 快照后开始生效。

如需调试，可临时设置：

```bash
Zzz_BILI_CRON_GUARD_MODE=snapshot
```

或：

```bash
Zzz_BILI_CRON_GUARD_MODE=restore
```

然后手动运行 `Zzz-Bili 定时保护`。正常使用时不要设置该变量。

青龙实际执行时间最终仍以面板中显示的 Cron 和青龙容器时区为准。
